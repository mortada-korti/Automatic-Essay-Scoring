...
from transformers import TrainingArguments, ElectraTokenizer, ElectraConfig, Trainer
...
# Import MoE and ELECTRA model components from custom ELECTRA_utils script
from scripts.ELECTRA.ELECTRA_utils import (
    freeze_electra_layers,
    ElectraLayerWithMoE,
    MoeElectraScorer,
    MoeElectraModel,
    MoEFeedForward,
...
config_space = {
    ...
    "unfrozen_layers": [2],            # How many top ELECTRA layers to fine-tune
...
results_path = Path(f"{RESULTS_DIR}/ELECTRA/ELECTRA_results.csv")  # Path to CSV where previous results are saved
...
# ### Run Cross-Prompt MoE-ELECTRA Experiments Over All Configurations
...
tokenizer = ElectraTokenizer.from_pretrained("google/electra-base-discriminator")  # Load ELECTRA tokenizer
...
for config_id, values in enumerate(config_list):
    ...
    for split in splits:
        ...
        # Load ELECTRA config and initialize MoeELECTRA model with MoE layers
        electra_config = ElectraConfig.from_pretrained("google/electra-base-discriminator")
        moe_model = MoeElectraModel(electra_config, num_experts=config["num_experts"], top_k=config["top_k"])
        model = MoeElectraScorer(base_model=moe_model, dropout=config["dropout"], feature_dim=n_handcrafted_features)
        model.to(device)

        # Freeze lower layers of ELECTRA
        freeze_electra_layers(model.encoder, num_unfrozen=config["unfrozen_layers"])
    ...
    pd.DataFrame(results).to_csv(f"{RESULTS_DIR}/ELECTRA/ELECTRA_results.csv", index=False)
