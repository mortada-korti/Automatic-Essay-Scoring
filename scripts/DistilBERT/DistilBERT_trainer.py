...
from transformers import TrainingArguments, DistilBertTokenizer, DistilBertConfig, Trainer
...
# Import MoE and DistilBERT model components from custom DistilBERT_utils script
from scripts.DistilBERT.DistilBERT_utils import (
    freeze_distilbert_layers,
    DistilBertLayerWithMoE,
    MoeDistilBertScorer,
    MoeDistilBertModel,
    MoEFeedForward,
)
...
results_path = Path(f"{RESULTS_DIR}/DistilBERT/DistilBERT_results.csv")  # Path to CSV where previous results are saved
...
# ### Run Cross-Prompt MoE-DistilBERT Experiments Over All Configurations
...
for config_id, values in enumerate(config_list):
    ...
    for split in splits:
        ...
        # Load DistilBERT config and initialize MoeDistilBERT model with MoE layers
        distilbert_config = DistilBertConfig.from_pretrained("distilbert-base-uncased")
        moe_model = MoeDistilBertModel(distilbert_config, num_experts=config["num_experts"], top_k=config["top_k"])
        model = MoeDistilBertScorer(base_model=moe_model, dropout=config["dropout"], feature_dim=n_handcrafted_features)
        model.to(device)

        # Freeze all but top n layers of the transformer
        freeze_distilbert_layers(model.encoder, num_unfrozen=config["unfrozen_layers"])
    ...
    pd.DataFrame(results).to_csv(f"{RESULTS_DIR}/DistilBERT/DistilBERT_results.csv", index=False)
