...
from transformers import TrainingArguments, RobertaTokenizer, RobertaConfig, Trainer
...
# Import MoE and RoBERTa model components from custom RoBERTa_utils script
from scripts.RoBERTa.RoBERTa_utils import (
    freeze_roberta_layers,
    RobertaLayerWithMoE,
    MoeRobertaScorer,
    MoeRobertaModel,
    MoEFeedForward,
...
# ### Define Hyperparameter Search Spac
...
config_space = {
    ...
    "unfrozen_layers": [2],            # How many top RoBERTa layers to fine-tune
...
results_path = Path(f"{RESULTS_DIR}/RoBERTa/RoBERTa_results.csv")  # Path to CSV where previous results are saved
...
# ### Run Cross-Prompt MoE-RoBERTa Experiments Over All Configurations
...
for config_id, values in enumerate(config_list):
    ...
    for split in splits:
        ...
        # Load RoBERTa config and initialize MoeRoBERTa model with MoE layers
        roberta_config = RobertaConfig.from_pretrained("roberta-base")
        moe_model = MoeRobertaModel(roberta_config, num_experts=config["num_experts"], top_k=config["top_k"])
        model = MoeRobertaScorer(base_model=moe_model, dropout=config["dropout"], feature_dim=n_handcrafted_features)
        model.to(device)

        # Freeze lower layers of RoBERTa
        freeze_roberta_layers(model.encoder, num_unfrozen=config["unfrozen_layers"])
    ...
    pd.DataFrame(results).to_csv(f"{RESULTS_DIR}/RoBERTa/RoBERTa_results.csv", index=False)
